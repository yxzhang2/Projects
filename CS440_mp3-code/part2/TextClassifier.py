            ...
            for i in range(len(train_set)):
                if (train_label[i]==c+1):
                    for w in range(len(vocab[c])):
                        self.likelihood[c][w]+=train_set[i].count(vocab[c][w])
 

        totalNum= [0]*self.num_class #Initialist list of all words
        
        for i in range(len(train_set)):
            totalNum[train_label[i]-1]+=len(train_set[i])
        

        for c in range (self.num_class):
            for w in range(len(vocab[c])):
                self.likelihood[c][w]=(1+self.likelihood[c][w])/(totalNum[c]+(len(vocab[c])))
            ...
            for c in range(self.num_class): #For every class
                ...
                for w in range(len(self.vocab[c])): #For every word in vocab list
                    ...
                    if self.vocab[c][w] in x_set[i]:
                        products[c]+=math.log(self.likelihood[c][w])*(x_set[i].count(self.vocab[c][w]))
                        #print (self.likelihood[c][w])
                        
                    # else :
                    #     products[c]+=math.log(len(vocab[c])
